...
public class EnhancedWidgetSprites {
    ...
    public SpriteData get(boolean active, boolean hovered) {
        if (!active) return sprites.getOrDefault(WidgetState.DISABLED, sprites.get(WidgetState.NORMAL));
        if (hovered) return sprites.getOrDefault(WidgetState.HOVERED, sprites.get(WidgetState.NORMAL));
