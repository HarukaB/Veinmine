...
public class ConfigToosAndProtectPage {
    ...
    public ConfigToosAndProtectPage(ConfigItems items) { this.items = items; }
    ...
    public List<AbstractWidget> build(int centerX) {
        ...
        widgets.add(new TitleWidget(layout.x(), layout.y(), Component.translatable("vm.config.screen.toolsandprotect")));
        layout.next(10);

        widgets.add(bool(layout, contentWidth, "vm.config.protect_tools", () -> items.protectTools, v -> items.protectTools = v));
        layout.next(20);

        widgets.add(bool(layout, contentWidth, "vm.config.protect_allValuable_tools", () -> items.protectAllDefaultValuableTools, v -> items.protectAllDefaultValuableTools = v));
        layout.next(20);

        durabilityThresholdSlider = intSlider(layout, contentWidth, "vm.config.durability_threshold", 1, 256, () -> items.durabilityThreshold, v -> items.durabilityThreshold = v, false);
        durabilityThresholdSlider.setCustomValueKey("vm.config.value.durability");
        widgets.add(durabilityThresholdSlider);
        layout.next(20);

        widgets.add(new TitleWidget(layout.x(), layout.y(), Component.translatable("vm.config.protected_tools")));
        layout.next(10);

        protectedToolsBox = new MultiLineEditBox(
        ...
        protectedToolsBox.setTooltip(Tooltip.create(Component.translatable("vm.config.protected_tools.tooltip")));
    ...
    public void save() {
        ...
        for (String line : protectedToolsBox.getValue().split("\n")) {
            if (!line.isBlank()) items.protectedTools.add(line.trim());
        }
    }
