...
public class ConfigAdvancedPage {
    ...
    public ConfigAdvancedPage(ConfigItems items) { this.items = items; }
    ...
    public List<AbstractWidget> build(int centerX) {
        ...
        widgets.add(new TitleWidget(layout.x(), layout.y(),
                Component.translatable("vm.config.screen.final_resort").setStyle(Style.EMPTY.withColor(ChatFormatting.RED))));
        layout.next(10);

        widgets.add(bool(layout, contentWidth, "vm.config.useIntrusiveCode", () -> items.useIntrusiveCode, v -> items.useIntrusiveCode = v));
        return widgets;
    }
