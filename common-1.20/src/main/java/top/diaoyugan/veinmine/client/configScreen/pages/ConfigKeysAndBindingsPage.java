...
public class ConfigKeysAndBindingsPage extends Screen {
    ...
    public List<AbstractWidget> build(int centerX) {
        ...
        widgets.add(new TitleWidget(layout.x(), layout.y(), Component.translatable("vm.config.screen.keysAndBinding")));
        layout.next(10);

        widgets.add(Button.builder(
                Component.translatable("vm.config.open_keybindings"),
                b -> Minecraft.getInstance().setScreen(new KeyBindsScreen(this, Minecraft.getInstance().options))
        ).bounds(layout.x(), layout.y(), contentWidth, 20)
         .tooltip(Tooltip.create(Component.translatable("vm.config.open_keybindings.tooltip")))
         .build());
        layout.next(20);

        widgets.add(bool(layout, contentWidth, "vm.config.use_hold_instead_of_toggle",
                () -> items.useHoldInsteadOfToggle, v -> items.useHoldInsteadOfToggle = v));
        layout.next(20);
