...
public class ConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        addTabButton(10, 30,  Component.translatable("vm.config.screen.main"), 20, 0, () -> showPage(0));
        addTabButton(10, 54,  Component.translatable("vm.config.screen.toolsandprotect"), 20, 1, () -> showPage(1));
        addTabButton(10, 78,  Component.translatable("vm.config.screen.highlights"), 20, 2, () -> showPage(2));
        addTabButton(10, 102, Component.translatable("vm.config.screen.keysAndBinding"), 20, 3, () -> showPage(3));
    ...
    private void addTabButton(int x, int y, Component text, int height, int index, Runnable action) {
    ...
    private void buildTabButtons() {
        ...
        for (PendingTab tab : pendingTabs) {
            Button btn = new TabButtonWidget(tab.x, tab.y, width, tab.height, tab.text, b -> tab.action.run());
            tabButtons.add(btn);
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float delta) {
        renderBackground(g);
        g.fill(0, height - 45, width, height, 0x55000000);
        g.fill(0, height - 45, width, height - 45 + 1, 0xFF888888);
        g.fillGradient(0, height - 45 - 1, width, height - 45, 0x00000000, 0x55000000);
        super.render(g, mouseX, mouseY, delta);
        g.drawString(this.font, Component.translatable("vm.config.screen.title"), 11, 14, 0xFFFFFFFF);
        g.drawCenteredString(this.font, Component.translatable("vm.config.saveTip"), width / 2, height - 40, 0xFFFFFFFF);
    }
